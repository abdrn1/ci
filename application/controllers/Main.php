...
class Main extends CI_Controller
{
    ...
    public $sms_username = 'anwarsoltan';
    public $sms_pass = '9853571';
    public $sms_sender = 'anwarsoltan';
    ...
    {
        $this->load->view('pages/admin/print_bill');
    }
    ...
    public function employee_delete()
    {
        ...
        if (empty($_POST['emp_id'])) {
            ...
            $results['msg'] = 'معرف المستخدم فارغ';
            echo json_encode($results);
        }

        if ($results['status'] !== 'error') {
            $results['rslt'] = $this->home_model->employee_delete($_POST['emp_id']);
    ...
    public function employee_update()
    {
        ...
        if (empty($_POST['emp_username'])) {
            $uploade['status'] = 'error';
            $uploade['msg'] = 'اسم الموظف فارغ';
            die(json_encode($uploade));
        }
        if (empty($_POST['emp_userid'])) {
            $uploade['status'] = 'error';
        ...
        if (empty($_POST['emp_password'])) {
            $uploade['status'] = 'error';
            $uploade['msg'] = 'كلمة مرور فارغة';
            die(json_encode($uploade));
        }

        if (empty($_POST['emp_id'])) {
            $uploade['status'] = 'error';
            $uploade['msg'] = 'معرف الموظف فارغ';
            die(json_encode($uploade));
        }
        ...
        if ($uploade['status'] != 'error') {
            if ($this->home_model->employee_update($emp_id, $emp_userid, $emp_username, $emp_password) == TRUE) {
                $uploade['status'] = 'Success';
                $uploade['msg'] = 'تم الإضافة بنجاح';
                echo json_encode($uploade);
            } else {
    ...
    public function employee_insert()
    {
        ...
        if ($uploade['status'] != 'error') {
            if ($this->home_model->employee_insert($emp_userid, $emp_username, $emp_password) == TRUE) {
                $uploade['status'] = 'Success';
                $uploade['msg'] = 'تم الإضافة بنجاح';
                echo json_encode($uploade);
            } else {
    ...
    public function employee_get_all()
    {
        $results = $this->home_model->employee_get_all();
    ...
    public function employee_get_selected()
    {
        $results['status'] = '';
        $results['msg'] = '';
        if (empty($_POST['emp_id'])) {
            $results['status'] = 'error';
            $results['msg'] = 'معرف الموظف فارغ';
            echo json_encode($results);
        }
        if ($results['status'] != 'error') {
            $results = $this->home_model->employee_get_selected($_POST['emp_id']);
            //$results = $this->security->xss_clean(json_decode(json_encode($results), true));
            if (empty($results)) {
                $results['status'] = 'error';
                $results['msg'] = 'لا يوجد بيانات';
     ...
     public function customer_insert()
    {
        ...
        if ($uploade['status'] != 'error') {
            if ($this->home_model->customer_insert($customer_name, $customer_jawwal,$customer_price, $customer_serial_number, $customer_street) == TRUE) {
                $uploade['status'] = 'Success';
                $uploade['msg'] = 'تم الإضافة بنجاح';
                echo json_encode($uploade);
            } else {
    ...
    public function customer_update()
    {
        ...
        if ($uploade['status'] != 'error') {
            if ($this->home_model->customer_update($customer_name, $customer_jawwal,$customer_price, $customer_serial_number, $customer_street, $customer_id) == TRUE) {
                $uploade['status'] = 'Success';
                $uploade['msg'] = 'تم الإضافة بنجاح';
                echo json_encode($uploade);
            } else {
    ...
    public function period_insert()
    {
        ...
        if ($uploade['status'] != 'error') {
            ...
            if ($uploade['complete'] == TRUE) {
                ...
                $uploade['status'] = 'error';
                $uploade['msg'] = 'لم يتم ادخال البيانات، قد يكون هناك سجل مسبق بنفس التاريخ';
                echo json_encode($uploade);
            }
        } else {
            echo json_encode($uploade);
    ...
    public function sendBillSMS($bill_consumption_sms, $bill_reminder_sms, $smsTextExtra, $customer_id)
    {
        ...
        if ($customer_jawwal != 0 && !empty($customer_jawwal)) {
            ...
            $url = 'http://www.hotsms.ps/sendbulksms.php?user_name=' . $this->sms_username . '&user_pass=' . $this->sms_pass . '&sender=' . $this->sms_sender . '&mobile=' . $jawwal . '&type=0&text=' . $messageencode;
    ...
    public function sendSMSLoop()
    {
        ...
        foreach ($customer_result as $value) {
            ...
            if (!empty($value->customer_jawwal) && strlen($value->customer_jawwal) == 9) {
                ...
                if ($messageType == 1) {
                    ...
                    $message = 'مشتركنا العزيز';
                    $message .= ' ';
                    $message .= ' المبلغ المطلوب لدورة هذا الشهر هو ';
                    $message .= $bill_reminder;
                    $message .= ' شيكل ';
                    $message .= 'عن ';
                    $message .= $bill_consumption;
    ...
    public function sendSMSAbd()
    {
        $messageArabic = 'مرحبا عبد';
        $messageencode = urlencode($messageArabic);

        $url = 'http://www.hotsms.ps/sendbulksms.php?user_name=anwarsoltan&user_pass=9853571&sender=anwarsoltan&mobile=972566100049'  . '&type=0&text=' . $messageencode;
    ...
    public function smsGetCredits()
    {
        ...
        $url = 'http://www.hotsms.ps/getbalance.php?user_name=' . $this->sms_username . '&user_pass=' . $this->sms_pass;
    ...
    public function bill_get_credit()
    {
        $results['status'] = '';
        $results['msg'] = '';
        ...
        $bill_date_from = $_POST['bill_date_from'];
       // log_message('bill_date_from', $bill_date_from);
        $bill_date_to = $_POST['bill_date_to'];
        $bill_region = $_POST['bill_region'];
        $bill_street = $_POST['bill_street'];
        $bill_is_paid = $_POST['bill_is_paid'];
        $credit_from = $_POST['credit_from'];
        $credit_to = $_POST['credit_to'];
        $results = $this->home_model->bill_get_credit($bill_customer_id, strtotime($bill_date_from), strtotime($bill_date_to), $bill_region, $bill_street, $bill_is_paid,$credit_from,$credit_to);
        // $results = $this->security->xss_clean(json_decode(json_encode($results), true));
        if (empty($results)) {
            $results['status'] = 'error';
            $results['msg'] = 'لا يوجد بيانات';
    ...
    public function bill_get_print2()
    {
        $results['status'] = '';
        $results['msg'] = '';
       ...
       // log_message('bill_date_from', $bill_date_from);
        ...
        $bill_street = $_POST['bill_street'];
        $bill_is_paid = $_POST['bill_is_paid'];
        $results = $this->home_model->bill_get_all2($bill_customer_id, strtotime($bill_date_from), strtotime($bill_date_to), $bill_region, $bill_street, $bill_is_paid);
        // $results = $this->security->xss_clean(json_decode(json_encode($results), true));
        if (empty($results)) {
            $results['status'] = 'error';
            $results['msg'] = 'لا يوجد بيانات';
    ...
    public function bill_get_print2_get()
    {


        $bill_customer_id = $_GET['bill_customer_id'];

        $bill_date_from =  $_GET['bill_date_from'];
        $bill_date_to = $_GET['bill_date_to'];
        $bill_region =  $_GET['bill_region'];
        $bill_street =  $_GET['bill_street'];
        $bill_is_paid = $_GET['bill_is_paid'];

        // echo $bill_customer_id;
        // return;
        ...
        if (empty($results)) {
            ...
            $this->load->view('pages/admin/print_bill', $data);
        }
    }
    ...
    public function get_credit_report()
    {
        ...
        $bill_street =  $_GET['bill_street'];
        $bill_is_paid = $_GET['bill_is_paid'];
        $credit_from = $_GET['credit_from'];
        $credit_to = $_GET['credit_to'];
        ...
        if (empty($results)) {
            ...
            $this->load->view('pages/admin/print_credit', $data);
        }
    }
    ...
    public function payment_insert()
    {
        ...
        if(!is_numeric($_POST['payment_value'])){
            $uploade['status'] = 'error';
            $uploade['msg'] = ' قيمة الدفع ليست رقماً';
            die(json_encode($uploade));  
        }
    ...
    public function cost_update()
    {
        ...
        if ($uploade['status'] != 'error') {
            if ($this->home_model->cost_update($cost_price, $send_sms,$min_price) == TRUE) {
                $uploade['status'] = 'Success';
                $uploade['msg'] = 'تم الإضافة بنجاح';
                echo json_encode($uploade);
            } else {
    ...
    public function period_consum_get_all_print()
    {
        $results['status'] = '';
        $results['msg'] = '';

        $period_region = $_GET['period_region'];
        $period_street = $_GET['period_street'];
        $results = $this->home_model->period_consum_get_all($period_region, $period_street);
        //$results = $this->security->xss_clean(json_decode(json_encode($results), true));
        if (empty($results)) {
            $results['status'] = 'error';
            ...
            $data['results'] = $results;
            $data['count'] = count($results);
            $this->load->view('pages/admin/print_consume', $data);
    ...
    public function period_consum_get_all_print2()
    {
        $results['status'] = '';
        $results['msg'] = '';
        ...
        if (empty($results)) {
            ...
            $data['results'] = $results;
            $data['count'] = count($results);
            $this->load->view('pages/admin/print_period_collect_mony', $data);

           // echo json_encode($results);
        }
    }

    public function period_consum_get_all_print3()
    {
        $results['status'] = '';
        $results['msg'] = '';
        ...
        if (empty($results)) {
            ...
            $data['results'] = $results;
            $data['count'] = count($results);
            $this->load->view('pages/admin/print_customer_list', $data);
