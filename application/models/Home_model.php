...
class Home_model extends CI_Model {
    ...
    public $temp_payment_paid_from_prev;
    public $temp_payment_discount_from_prev;
...
//////////////////// Region Page ////////////
    ...
    function employee_insert($userid, $username,$password) {
    ...
    function employee_update($id, $userid,$username, $password) {
    ...
    function employee_get_selected($employee_id) {
    ...
    function region_insert($region_name, $sender_name) {
    ...
    function region_update($region_name, $sender_name, $region_id) {
    ...
    function region_get_all($regionname ='') {
        $this->db->select('*'); // <-- There is never any reason to write this line!
        $this->db->from('region');
     
        
            if($regionname!='Admin' && !empty($regionname)){
               
                    $query = $this->db->where('region_name', $regionname); 
...
//////////////////// customer Page ////////////
    ...
    function customer_insert($customer_name, $customer_jawwal,$customer_price, $customer_serial_number, $customer_street) {
        $customer_max_id = 00;
    ...
    function customer_insert_mobile($customer_name, $customer_jawwal, $customer_order, $customer_street) {
        $customer_max_id = 00;
        $this->db->select_max('customer_id', 'customer_max_id'); // <-- There is never any reason to write this line!
        $this->db->from('customer');
        $query = $this->db->get();
    ...
    function customer_update_mobile($customer_name, $customer_jawwal, $customer_order, $customer_street, $customer_id) {
    ...
    function customer_update($customer_name, $customer_jawwal,$customer_price, $customer_serial_number, $customer_street, $customer_id) {
...
//////////////////// Period Page ////////////
    ...
    function period_get_all_list_by_customer($period_region, $period_street, $customer_id) {
        $this->db->select('*'); // <-- There is never any reason to write this line!
        $this->db->from('customer');
        $this->db->join('(select max(period_id) as period_max_id ,period_customer from period group by period_customer) as period_max', 'period_customer = customer_id', 'left');
        $this->db->join('(select sum(bill_reminder) as bill_sum ,customer_id as customer_sum_id from bill,period,customer where customer_id = period_customer AND period_id = bill_period group by customer_id) as bill_reminder_sum', 'customer_sum_id = customer_id', 'left');
...
//        }
    ...
    function period_insert($period_date, $period_price, $period_customer, $period_reading, $period_reading_date, $period_previous_reading, $period_note, $payment_value, $payment_discount) {
        ...
        // get customer specific cost
        $this->db->select('*'); 
        $this->db->from('customer');
        $this->db->where('customer_id', $period_customer);
        $query = $this->db->get();
        foreach ($query->result_array() as $row) {
            $cost_price = $row['customer_price'];
        }
        // end of get customer specific cost
            ...
            $query = $this->db->get();
            foreach ($query->result_array() as $row) {
                $server_price = $row['cost_price'];
                $min_price = $row['min_price'];
            }

           

            if (empty($period_price) || $period_price==0) {
               
              if($cost_price==0){
                $cost_price=$server_price;
              }
        ...
        if($bill_reminder<$min_price){
            $bill_reminder=$min_price;
        }

        $trans['bill_sum'] =$bill_reminder;
        ...
        if(empty($period_note)){
            $period_note = 'تم الدفع من صفحة الأدخال المبسط';
        }


        $payment_insert = $this->payment_insert($bill_id, $payment_value, $payment_discount,$period_note);
    
        $payment_insert['period_id'] = $period_id;
        $payment_insert['bill_sum'] =$bill_reminder;
    ...
    function payment_insert($bill_id, $payment_value, $payment_discount, $payemnt_note) {
        ...
        $this->reminder['payment'] = 0;
        $this->reminder['discount'] = 0;
        $in = FALSE;
        $payment_prev_arr = $this->payment_insert_from_prev($bill_id);


        $payment_value += abs($payment_prev_arr['totall_prev_amount']);
        $payment_discount += abs($payment_prev_arr['totall_prev_discount']);


        // $reminder_again = array();
        $this->reminder = $this->payment_insert_prev($bill_id, $payment_value, $payment_discount, $payemnt_note);

        if ($this->reminder['temp_bill_reminder'] < 0) {

            $payment_prev_arr = $this->payment_insert_from_prev($bill_id);
            $this->reminder['payment'] += abs($payment_prev_arr['totall_prev_amount']);
            $this->reminder['discount'] += abs($payment_prev_arr['totall_prev_discount']);

            $this->reminder = $this->payment_insert_prev($bill_id, $this->reminder['payment'], $this->reminder['discount'], $payemnt_note);
        }

        $this->db->trans_start();
        $this->db->trans_strict();

        // $reminder = $reminder_again;

        if ($this->reminder['payment'] > 0 || $this->reminder['discount'] > 0) {
            $data = array(
                'payment_bill' => $bill_id,
                'payment_value' => $this->reminder['payment'],
                'payment_discount' => $this->reminder['discount'],
            ...
            $total_paid = ($this->reminder['payment'] + $this->reminder['discount']);

            $this->db->set('bill_reminder', 'bill_reminder -' . round($total_paid, 2), FALSE);
            $this->db->set('bill_discount', 'bill_discount +' . round($this->reminder['discount'], 2), FALSE);
            $this->db->set('bill_paid', 'bill_paid +' . round($this->reminder['payment'], 2), FALSE);
        ...
        $trans['bill_id'] = $bill_id;
        $trans['note'] = 'error';
        $trans['rem'] = $this->reminder['payment'];
        $trans['in'] = $in;
        $trans['disc'] = $payment_discount;
        $trans['reminder'] = $this->reminder;
        return $trans;
    }
    ...
    function payment_insert_prev($bill_id, $payment, $discount, $payemnt_note) {
        ...
        foreach ($bills_reminder as $row) {
            ...
            if ($discount != 0) {
                ...
                if ($temp_bill_reminder >= $discount) {
                    $temp_bill_reminder -= $discount;
                    $temp_discount = $discount;
                    $discount = 0;
                } else if ($temp_bill_reminder < $discount) {
                    $temp_discount = $temp_bill_reminder;
                    $discount = $discount - $temp_bill_reminder;
                    $temp_bill_reminder = 0;
                }

                $data = array(
                    'payment_bill' => $temp_bill_id,
                    'payment_value' => round(00, 2),
    ...
    function payment_insert_from_prev($bill_id) {
        ...
        $totall_prev_discount = 0.0;

//        global $this->temp_payment_paid_from_prev;
//        global $temp_payment_discount;

        foreach ($bills_reminder_to_customer as $row) {
            $dsicountside = FALSE;
            $temp_bill_id = $row->bill_id;
            $temp_bill_reminder = $row->bill_reminder;
            $temp_bill_dsicount = $row->bill_discount;
            $temp_bill_paid = $row->bill_paid;
            $temp_bill_reminder_abs = abs($temp_bill_reminder);

            // $totall_prev_amount += $temp_bill_reminder_abs;
            $temp_payment_reminder = $temp_bill_reminder_abs;
            $this->temp_payment_paid_from_prev = 0;
            $this->temp_payment_discount_from_prev = 0;

            if ($temp_bill_reminder < 0) {
                if ($temp_bill_paid > 0) {
                    if ($temp_bill_reminder_abs > $temp_bill_paid) {
                        $temp_bill_reminder_abs -= $temp_bill_paid;
                        $this->temp_payment_paid_from_prev = $temp_bill_paid;
                        $temp_bill_paid = 0;
                    } else {
                        $temp_bill_paid -= $temp_bill_reminder_abs;
                        $this->temp_payment_paid_from_prev = $temp_bill_reminder_abs;
                        $temp_bill_reminder_abs = 0;
                    }
                }

                if ($temp_bill_dsicount > 0) {
                    if ($temp_bill_reminder_abs > $temp_bill_dsicount) {
                        $temp_bill_reminder_abs -= $temp_bill_dsicount;
                        $this->temp_payment_discount_from_prev = $temp_bill_dsicount;
                        $temp_bill_dsicount = 0;
                    } else {
                        $temp_bill_dsicount -= $temp_bill_reminder_abs;
                        $this->temp_payment_discount_from_prev = $temp_bill_reminder_abs;
                        $temp_bill_reminder_abs = 0;
                    }
                }
            }
            $totall_prev_amount += $this->temp_payment_paid_from_prev;
            $totall_prev_discount += $this->temp_payment_discount_from_prev;

            $this->db->set('bill_reminder', 'bill_reminder +' . round($temp_payment_reminder, 2), FALSE);
            $this->db->set('bill_discount', 'bill_discount -' . round($this->temp_payment_discount_from_prev, 2), FALSE);
            $this->db->set('bill_paid', 'bill_paid -' . round($this->temp_payment_paid_from_prev, 2), FALSE);
            $this->db->where('bill_id', $temp_bill_id);
            $this->db->update('bill');

            $payments = $this->bill_get_payment_for_transfer($temp_payment_reminder, $temp_bill_id);
            //var_dump($payments);
            $payment_discount_temp_enough = -1;
            $payment_paid_temp_enough = -1;
            foreach ($payments as $payment_row) {
                $payment_id = $payment_row->payment_id;
                $payment_discount_temp = $payment_row->payment_discount;
                $payment_paid_temp = $payment_row->payment_value;
               // echo 'Id ' . $payment_id . ' discount ' . $this->temp_payment_discount_from_prev . ' paid ' . $this->temp_payment_paid_from_prev . ' ';
                $temp_discount_for_payment = 0;
                $temp_paid_for_payment = 0;

               // echo '2- Id ' . $payment_id . ' discount ' . $this->temp_payment_discount_from_prev . ' paid ' . $this->temp_payment_paid_from_prev . ' ';
               // echo '####' . $payment_discount_temp . ' #### ' . $payment_paid_temp . ' ';
                
               // echo '#**#' . $payment_discount_temp_enough . ' #**# ' . $payment_paid_temp_enough . ' ';
                
                if($payment_discount_temp_enough != 0 || $payment_paid_temp_enough != 0) {
                    if ($this->temp_payment_discount_from_prev != 0 || $this->temp_payment_paid_from_prev != 0) {
                      //  echo $this->temp_payment_discount_from_prev . ' ************ ' . $this->temp_payment_paid_from_prev;
                        if ($this->temp_payment_discount_from_prev >= $payment_discount_temp) {
                            $this->temp_payment_discount_from_prev -= $payment_discount_temp;
                            $temp_discount_for_payment = $payment_discount_temp;
                            $payment_discount_temp = 0;
                           // echo 'in dsicount0 ' . $temp_discount_for_payment;
                        } else if ($this->temp_payment_discount_from_prev < $payment_discount_temp) {
                            $payment_discount_temp -= $this->temp_payment_discount_from_prev;
                            $temp_discount_for_payment = $this->temp_payment_discount_from_prev;
                            $this->temp_payment_discount_from_prev = 0;
                            $payment_discount_temp_enough = 0;
                           // echo 'in dsicount1 ' . $temp_discount_for_payment ;
                        }
                        
                        if ($this->temp_payment_paid_from_prev >= $payment_paid_temp) {
                            $this->temp_payment_paid_from_prev -= $payment_paid_temp;
                            $temp_paid_for_payment = $payment_paid_temp;
                            $payment_paid_temp = 0;
                           // echo 'in paid0 ' . $temp_paid_for_payment;
                        } else if ($this->temp_payment_paid_from_prev < $payment_paid_temp) {
                            $payment_paid_temp -= $this->temp_payment_paid_from_prev;
                            $temp_paid_for_payment = $this->temp_payment_paid_from_prev;
                            $this->temp_payment_paid_from_prev = 0;
                            $payment_paid_temp_enough = 0;
                           // echo 'in paid1' . $temp_paid_for_payment;
                        }
                        if ( $payment_discount_temp == 0 && $payment_paid_temp == 0) {
                           // echo 'in delete ' . $temp_discount_for_payment . ' ^^^ ' . $temp_paid_for_payment . ' $$ ' . $payment_id.' /// ';
                            $this->db->where('payment_id', $payment_id);
                            $this->db->delete('payment');
                        } else {
                            //echo 'in dis' . $temp_discount_for_payment . ' ^^^ ' . $temp_paid_for_payment . ' $$ ' . $payment_id.' /// ';
                            $this->db->set('payment_discount', 'payment_discount -' . round($temp_discount_for_payment, 2), FALSE);
                            $this->db->set('payment_value', 'payment_value -' . round($temp_paid_for_payment, 2), FALSE);
                            $this->db->where('payment_id', $payment_id);
            ...
            $temp_payment_reminder = 0;
        }
        $this->db->trans_complete();

        $totall_prev_arrs['totall_prev_amount'] = $totall_prev_amount;
        $totall_prev_arrs['totall_prev_discount'] = $totall_prev_discount;
        return $totall_prev_arrs;
    }
...
function bill_get_credit($customer_id,$bill_date_from, $bill_date_to, $bill_region, $bill_street, $bill_is_paid,$credit_from,$credit_to) {
...
if ($credit_from!= '') {
        $this->db->where('bill_sum >',$credit_from );
    }
    if ($credit_to!= '') {
        $this->db->where('bill_sum <=',$credit_to );
    }
...
function bill_get_all2($customer_id,$bill_date_from, $bill_date_to, $bill_region, $bill_street, $bill_is_paid) {
    $this->db->select('*'); // <-- There is never any reason to write this line!
    $this->db->from('bill');
    $this->db->join('period', 'period_id = bill_period');
    $this->db->join('customer', 'period_customer = customer_id');
    $this->db->join('(select sum(bill_reminder) as bill_sum ,customer_id as customer_sum_id from bill,period,customer where customer_id = period_customer AND period_id = bill_period group by customer_id) as bill_reminder_sum', 'customer_sum_id = customer_id');
    $this->db->join('street', 'customer_street = street_id');
    $this->db->join('region', 'street_region = region_id');
    //$bill_date_from='';
   // $bill_date_to='';
    ...
    function bill_get_payment_for_transfer($transfer_value, $bill_id) {
        ...
        $this->db->order_by('payment_value', 'ASC');
        $this->db->order_by('payment_discount', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
...
//////////////////// ***************** ////////////
    function cost_update($cost_price,$send_sms,$min_price) {

        $this->db->set('cost_price', $cost_price);
        $this->db->set('send_sms', $send_sms);
        $this->db->set('min_price', $min_price);
...
/////////////////////////////////////
    ...
    function period_consum_get_all($period_region, $period_street) {
        ...
        if(count($result) > 0){
            foreach ($result as $key => $value){
                $result[$key]['payment'] = $this->payment_get_selected_bill($value['bill_id']);
                $result[$key]['add_cost'] = $this->get_add_cost($value['period_id']);
            }
        }
        return $result;
    }
    ...
    function get_payment_list($payment_list_date,$payment_list_end_date,$bill_street,$bill_region) {
        ...
        if($payment_list_end_date != ''){
        $this->db->where('STR_TO_DATE(payment_date, "%Y-%m-%d") <=', date('Y-m-d', $payment_list_end_date));
        }
        if ($bill_street == -1) {
            if ($bill_region != -1) {
                $this->db->where_in('street_region', $bill_region);
...
//////////////////// ***************** ////////////
    ...
    function getRegionStreets($region_id){
        $this->db->select('*'); // <-- There is never any reason to write this line!
        $this->db->from('street');
        $this->db->where('street_region',$region_id);
        $query =  $this->db->get();
        return $query->result();
    }

    function GetPeriodForAllCustomers($period_date, $period_region, $period_street,$screenType) {
        $this->db->select('*'); // <-- There is never any reason to write this line!
        $this->db->from('customer');
        $this->db->join('(select max(period_id) as period_max_id ,period_customer from period group by period_customer) as period_max', 'period_customer = customer_id', 'left');
        $this->db->join('(select sum(bill_reminder) as bill_sum ,customer_id as customer_sum_id from bill,period,customer where customer_id = period_customer AND period_id = bill_period group by customer_id) as bill_reminder_sum', 'customer_sum_id = customer_id', 'left');
        ...
        //$this->db->where('period_date', date('Y-m-d', $period_date));
         ...
         if($period_date!=-1){
            ...
            //working nice for use later
           if($screenType==0){
               $insert_date =date('Y-m-d',strtotime($period_date));
             $this->db->where(" ( period_reading_date<>'".$insert_date."' or period_reading_date is null) ");
        ...
        if($screenType==1){//1: payment screeen 
            $this->db->where('bill_sum>', 1);
        }
