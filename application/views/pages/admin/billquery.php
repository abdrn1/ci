                    ...
                    <div class="col-md-10 col-md-offset-1">
                        ...
                        <div class="form-group">
                            ...
                            <div class="clearfix"></div>-->
                                ...
                                <hr>
                                <div class="form-inline">
                                    <label for="bill_region" class="col-md-2 control-labeli">الفاتورة مدفوعة او لا</label>
                                    ...
                                    <div class="col-md-4">
                                        <select class="form-control" id="bill_is_paid">
                                            ...
                                            <option selected id="paid_2">غير مدفوع</option>
                                        </select>
                                    </div>
                                ...
                                <?php
                                    ...
                                    $minus10days = $currentDate->sub(new DateInterval('P10D'));
                                    
                                    //Get yesterday's date in a YYYY-MM-DD format.
                                    $minus10days = $minus10days->format('Y-m-d');
                                    ?>
                                <div class="form-inline">
                                    <label for="bill_date_from" class="col-md-2 control-labeli">من تاريخ</label>
                                    <div class="col-md-4">
                                        <input type="date" value="<?php echo $minus10days;  ?>" class="form-control" id="bill_date_from" name="bill_date_from">
                                    </div>
                                </div>
                                <div class="form-inline">
                                    <label for="bill_date_to" class="col-md-2 control-labeli">حتى تاريخ</label>
                                    <div class="col-md-4">
                                  
                                        <input type="date" value="<?php echo date('Y-m-d'); ?>" class="form-control" id="bill_date_to" name="bill_date_to">
                                    </div>
                                </div>
                                <hr>
                                <div class="clearfix"></div>
            ...
            $(document).ready(function() {
                ...
                displayRecords = function(arr) {
                    ...
                    $.each(arr, function(key, record) {
                        currRow = `<tr>
                        <td><input type="checkbox" value="period_id" name ="check_periods[]" /></td>
                        <td>${record.customer_name}</td>
                        <td>${record.region_name}</td>
                        <td>${record.street_name}</td>
                        <td>${record.period_date}</td>
                        <td>${record.period_reading_date}</td>
                        <td>${record.period_previous_reading}</td>
                        <td>${record.period_reading}</td>
                        <td>${record.bill_consumption}</td>
                        <td>${record.bill_price}</td>
                        <td>${record.bill_paid}</td>
                        <td>${record.bill_discount}</td>
                        <td>${record.bill_reminder}</td>
...
`;
            ...
            bill_get_print = function(bill_customer_id, bill_date_from, bill_date_to, bill_region, bill_street, bill_is_paid) {
                $('.loadingimg').show();
                $('.loadingsection').hide();
                $('#table_query_result tbody').html('');
                $.ajax({
                    url: '<?php echo base_url(); ?>main/bill_get_print2',
                    type: 'post',
                    data: {
            ...
            bill_get_print_url = function(bill_customer_id, bill_date_from, bill_date_to, bill_region, bill_street, bill_is_paid) {
                

                url = '<?php echo base_url(); ?>';
                console.log('bill date from :'+bill_date_from);
                console.log('bill date to :'+bill_date_to);
                url+=`main/bill_get_print2_get?bill_customer_id=${bill_customer_id}&bill_date_from=${bill_date_from}
            ...
            $('#btn_query').click(function(e) {
                ...
                bill_get_print($('#customer_bill_customer').children("option:selected").prop('id'), $('#bill_date_from').val(), $('#bill_date_to').val(), $('#bill_region').children("option:selected").prop('id').split('_').pop(), $('#bill_street').children("option:selected").prop('id').split('_').pop(),
                    $('#bill_is_paid').children("option:selected").prop('id').split('_').pop());
            });
            ...
            $('#btn_print').click(function(e) {
                // call get period date
                bill_get_print_url($('#customer_bill_customer').children("option:selected").prop('id'), $('#bill_date_from').val(), $('#bill_date_to').val(), $('#bill_region').children("option:selected").prop('id').split('_').pop(), $('#bill_street').children("option:selected").prop('id').split('_').pop(),
                    $('#bill_is_paid').children("option:selected").prop('id').split('_').pop());
            });
        </script>